# PolyGraph JSON Model Specification
...
- **Phased timed actors:** `ebs` has `phase: 20` (fires 20 ms after the period start), `ifd` has `phase: 50`.
- **Mixed rates:** `c_ldr_obd` uses `rateSrc: "1/4"` so the lidar needs 4 firings to produce 1 full token for the obstacle detector.
- **Fan-in:** `spc` (Speed Control) consumes from three channels (`obd`, `odm`, `tsd`).
- **Fan-out:** `lcm` (Left Camera) produces to four different processing actors.
